// Copied verbatim from src/shared/sodax-service.ts for the self-contained Phase 2 showcase.
// Source of truth lives in src/shared/sodax-service.ts — keep in sync if that changes.
import {
  Sodax,
  EvmSpokeProvider,
...
export class SodaxBridgeService implements IBridgeService {
  ...
  async getQuote(params: SwapParams, maxAttempts = 5): Promise<BridgeQuote> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (attempt < maxAttempts) {
        console.log(
          `  Quote attempt ${attempt}/${maxAttempts} failed (${errMsg}) — retrying in 5s...`
        );
        await sleep(5000);
      } else {
  ...
  async pollStatus(statusHash: string, maxAttempts = 120): Promise<BridgePollResult> {
    console.log(`\n[Sodax] Polling status for: ${statusHash}`);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (statusResult.ok) {
        ...
        console.log(
          `  Attempt ${attempt}/${maxAttempts} — ${getStatusLabel(status)}`
        );

        if (status === SolverIntentStatusCode.SOLVED) {
          const fillTxHash = statusResult.value.fill_tx_hash as
            | `0x${string}`
            | undefined;
          ...
          if (fillTxHash) {
            try {
              const intentState =
                await this.sodax.swaps.getFilledIntent(fillTxHash);
              amountReceived = intentState.receivedOutput;
              console.log(`  Settled: ${amountReceived} stroops`);
            } catch {
              console.warn(`  Could not fetch settled amount for ${fillTxHash}`);
            }
          }

          let destTxHash = "SOLVED";
          if (fillTxHash) {
